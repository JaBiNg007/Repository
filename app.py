...
# --- คลาสและฟังก์ชันหลักของโปรแกรมจัดวางพื้นที่ (ส่วนที่คุณให้มาแต่แรก) ---
class Activity:
...
def calculate_layout_fingerprint(placed_activities_info):
    fingerprint_data = []
    sorted_info = sorted(placed_activities_info, key=lambda info: (info['position'][0], info['position'][1], info['placed_dimensions'][0], info['placed_dimensions'][1], info['name']))
    
    for info in sorted_info:
        fingerprint_data.append(
            (info['name'], info['placed_dimensions'][0], info['placed_dimensions'][1], info['position'][0], info['position'][1])
        )
...
def solve_packing_with_variety_and_rotation(board_width, board_height, activities_input_list):
    ...
    random.shuffle(current_attempt_activities) 
    ...
    for activity in current_attempt_activities:
        ...
        random.shuffle(all_possible_orientations)
...
def get_plot_as_base64_image(board_width, board_height, placed_activities_info, simulation_num, total_area_covered):
    ...
    plt.title(f'Simulation #{simulation_num}: Board {board_width}x{board_height} | Covered: {total_area_covered} ตร.หน่วย')
...
def index():
    if request.method == 'POST': # ถ้าเป็นการส่งข้อมูลจากฟอร์ม (Submit)
        try:
            ...
            colors_pool = ['#FFC300', '#FF5733', '#C70039', '#900C3F', '#581845', '#4CAF50', '#2196F3', '#FF9800', '#673AB7',
                           '#00BCD4', '#8BC34A', '#FFEB3B', '#795548', '#607D8B', '#E91E63', '#9C27B0', '#009688', '#FFCDD2']
            ...
            for i in range(num_activities):
                ...
                if not activity_name or required_area <= 0:
                    return render_template('index.html', error="ชื่อพื้นที่ย่อยต้องไม่ว่างเปล่าและขนาดพื้นที่ต้องเป็นจำนวนบวก")
            ...
            MAX_ATTEMPTS = num_simulations * 100 # เช่น ถ้าต้องการ 3 รูปแบบ จะพยายามสูงสุด 300 ครั้ง
            ...
            while successful_simulations_count < num_simulations and simulation_attempt_num < MAX_ATTEMPTS:
                ...
                final_board_grid, placed_info = solve_packing_with_variety_and_rotation(board_width, board_height, current_simulation_activities)
                ...
                if final_board_grid is not None: # ถ้าจัดวางสำเร็จ
                    ...
                    if current_layout_fingerprint not in generated_fingerprints: # ตรวจสอบว่าเป็นรูปแบบที่ไม่ซ้ำกันหรือไม่
                        ...
                        plot_image = get_plot_as_base64_image(board_width, board_height, placed_info, successful_simulations_count, total_required_area)
...
if __name__ == '__main__':
    app.run(debug=True) # debug=True จะช่วยให้เห็นข้อผิดพลาดได้ง่ายขึ้น และเว็บจะรีโหลดอัตโนมัติเมื่อโค้ดมีการเปลี่ยนแปลง
